...
use rusty_tesseract_custom::{Args, Image};
...
fn main() {
    ...
    let tesseract_version = rusty_tesseract_custom::get_tesseract_version().unwrap();
    println!("The tesseract version is: {}", tesseract_version);

    let tesseract_langs = rusty_tesseract_custom::get_tesseract_langs().unwrap();
    ...
    let parameters = rusty_tesseract_custom::get_tesseract_config_parameters().unwrap();
    println!(
        "Example config variable: {}",
        parameters.config_parameters.first().unwrap(),
    );

    let output = rusty_tesseract_custom::image_to_string(&img, &image_to_string_args).unwrap();
    ...
    let box_output = rusty_tesseract_custom::image_to_boxes(&img, &default_args).unwrap();
    println!(
    ...
    let data_output = rusty_tesseract_custom::image_to_data(&img, &default_args).unwrap();
...
mod tests {
    ...
    fn horizontal_text() {
        ...
        let default_args = Args::default();
        let output = rusty_tesseract_custom::image_to_string(&img, &default_args).unwrap();
